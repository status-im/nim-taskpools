...
const ULF_WAIT_MASK     = (ULF_NO_ERRNO |
	                         ULF_WAIT_WORKQ_DATA_CONTENTION |
	                         ULF_WAIT_CANCEL_POINT | ULF_WAIT_ADAPTIVE_SPIN)

const ULF_WAKE_MASK     = (ULF_NO_ERRNO |
	                         ULF_WAKE_ALL |
	                         ULF_WAKE_THREAD |
...
proc wait*(futex: var Futex, refVal: uint32) {.inline.} =
  ...
  discard ulock_wait(UL_COMPARE_AND_WAIT or ULF_NO_ERRNO, futex.value, uint64 refVal, 0)
...
proc wake*(futex: var Futex) {.inline.} =
  ...
  discard ulock_wake(UL_COMPARE_AND_WAIT or ULF_NO_ERRNO, futex.value, 0)

proc wakeAll*(futex: var Futex) {.inline.} =
  ## Wake all threads (from the same process)
  discard ulock_wake(UL_COMPARE_AND_WAIT or ULF_NO_ERRNO or ULF_WAKE_ALL, futex.value, 0)
